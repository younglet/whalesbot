"""drivers/ambient_light.py — 环境光传感器 driver
...
用法:
    python -m drivers.ambient_light       # 自检
    或在自己的脚本里:
        from link import SerialWrap
        from drivers.ambient_light import AmbientLight
        serial_obj = SerialWrap()
        al = AmbientLight(serial_obj, port=1)
        val = al.read()

协议帧:
    读取: struct.pack("<bbbH", 0x07, 0x04, port_id, 0)
    响应: dev_id(1) + mode(1) + port_id(1) + value(2, unsigned short)

实现参考:
    _backup/mc602_ctl2.py: ambient_light mode
"""
...
class AmbientLight:
    ...
    def __init__(self, serial, port=1):
        ...
        if not serial.device.name.startswith("mc602"):
            raise RuntimeError(f"ambient_light 仅支持 MC602，当前设备 {serial.device.name}")
        self._serial = serial
        self._port = port
...
if __name__ == "__main__":
    ...
    print("== AmbientLight 自检 ==")
    serial_obj = SerialWrap()

    for port in range(1, 5):
        al = AmbientLight(serial_obj, port=port)
        val = al.read()
